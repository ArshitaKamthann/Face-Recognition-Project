...
#videoStream = VideoStream(src=0, framerate=FPS).start()
...
#videoStream = imutils.resize(self.current_frame.read(), width=resolution[0], height=resolution[1])
#time.sleep(2.0) #Let the main thread sleep for 2 sec and than will be start video streaming
...
@app.route('/')
def index():
    return render_template('videostream.html')  #required html file in templates directory
...
@app.route('/video')
def video():
    return Response(generateFrames(videoStream),
...
def detect_faces_in_image(file_stream):
    ...
    if len(unknown_face_encodings) > 0:
        ...
        match_results = face_recognition.compare_faces(encodeList, unknown_face_encodings[0])
        match_resultsDis = face_recognition.face_distance(encodeList, unknown_face_encodings[0])
        ...
        if match_results[0] == True:
            ...
            face_match = True
            match_resultsDistense = "0.2" #match_resultsDis
